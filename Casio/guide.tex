...
\begin{enumerate}
	\item At the main \runmat screen, press \code{[OPTN]}, followed by \code{MAT} (\code{[F2]}). Press \code{$\triangleright$} (\code{[F6]}) twice, then select \code{Vct} (\code{[F1]}).
...
\begin{enumerate}
	\item In the vector section under \code{OPTN > MAT}, select \code{DotP} (\code{[F2]}).
...
\begin{enumerate}
	\item In the vector section under \code{OPTN > MAT}, select \code{CrsP} (\code{[F3]}).
...
\begin{enumerate}
	\item In the vector section under \code{OPTN > MAT}, select \code{Angle} (\code{[F4]}).
...
\begin{enumerate}
	\item In the vector section under \code{OPTN > MAT}, select \code{UntV} (\code{[F5]}).
